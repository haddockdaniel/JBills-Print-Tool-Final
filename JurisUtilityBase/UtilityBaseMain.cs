        ...
        private void listBoxCompanies_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            else
            {
                foreach (DataRow dr in myRSPC2.Tables[0].Rows)
                    comboBoxCli.Items.Add(dr["PC"].ToString());
                comboBoxCli.SelectedIndex = 0;
            }
        }
      ...
      //  }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (verifyBoxes())
            {
                ...
                UpdateStatus("Converting Invoices...This will take some time", runningTotal, total);
        ...
        private void checkBoxByCliMat_CheckedChanged(object sender, EventArgs e)
        {
            if (checkBoxByDate.Checked || checkBoxByNumber.Checked || checkBoxByCliMat.Checked || checkBoxBalance.Checked)
                ...
                button1.Enabled = false;
            comboBoxCli.Visible = checkBoxByCliMat.Checked;
            checkBoxByMat.Enabled = checkBoxByCliMat.Checked;
            
            checkOperative();
            if (!checkBoxByCliMat.Checked && checkBoxByMat.Checked)
                checkBoxByMat.Checked = false;
            if (!checkBoxByCliMat.Checked)
                comboBoxMat.Visible = false;
        }
            ...
            foreach (Control g in groupBox2.Controls) 
            {
                if (g is CheckBox)
                {
                    CheckBox cb = (CheckBox)g;
                        if (cb.Checked && !cb.Name.Equals("checkBoxByMat"))
                        checks++;
                }
        ...
        private List<Bill> getBills()
        {
            ...
            if (checkBoxByCliMat.Checked && !checkBoxByMat.Checked)
                whereClauses.Add(" dbo.jfn_FormatClientCode(clicode) = '" + this.comboBoxCli.GetItemText(this.comboBoxCli.SelectedItem).Split(' ')[0] + "' ");
            if (checkBoxByMat.Checked && checkBoxByCliMat.Checked)
                whereClauses.Add(" dbo.jfn_FormatClientCode(clicode) = '" + this.comboBoxCli.GetItemText(this.comboBoxCli.SelectedItem).Split(' ')[0] + "' and " +
                    " dbo.jfn_FormatMatterCode(matcode)= '" + this.comboBoxMat.GetItemText(this.comboBoxMat.SelectedItem).Split(' ')[0] + "'  ");
        ...
        private void checkBoxByMat_CheckedChanged(object sender, EventArgs e)
        {
            if (checkBoxByMat.Checked)
            {
                try
                {
                    if (!string.IsNullOrEmpty(this.comboBoxCli.GetItemText(this.comboBoxCli.SelectedItem).Split(' ')[0]))
                    {
                        comboBoxMat.Items.Clear();
                        comboBoxMat.Enabled = true;
                        comboBoxMat.Visible = true;
                        int i = 0;
                        //only list clients with invoices
                        string SQLPC2 = "select dbo.jfn_FormatMatterCode(matcode)  + '    ' + matreportingname as PC from matter " +
                                        " where matsysnbr in (select distinct matsysnbr " +
                                        " from ledgerhistory inner join arbill on arbillnbr = lhbillnbr  " +
                                        " inner join matter on matsysnbr = lhmatter inner join client on clisysnbr = matclinbr  " +
                                        " where lhtype in ('3', '4') and lhbillnbr not in (select lhbillnbr from ledgerhistory where lhtype in ('A', 'B', 'C'))" +
                                        " and dbo.jfn_FormatClientCode(clicode) = '" + this.comboBoxCli.GetItemText(this.comboBoxCli.SelectedItem).Split(' ')[0] + "' " +
                                         ") " +
                                        " order by dbo.jfn_FormatMatterCode(matcode)";
                        DataSet myRSPC2 = _jurisUtility.RecordsetFromSQL(SQLPC2);
                        ...
                        {
                            foreach (DataRow dr in myRSPC2.Tables[0].Rows)
                                comboBoxMat.Items.Add(dr["PC"].ToString());
